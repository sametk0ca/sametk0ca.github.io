...
cover:
    ...
    relative: false
related:
  - "[[concepts/software-supply-chain-security|Software Supply Chain Security & SLSA]]"
  - "[[Supply_Chain_Security|Yazılım Tedarik Zinciri Güvenliği (Supply Chain Security)]]"
  - "[[SBOMs|SBOM (Software Bill of Materials)]]"
  - "[[slsa-supply-chain-2024|SLSA Framework (2024)]]"
---
...
![Diyagram](/img/mermaid-software-supply-chain-security-1-86382df6.svg)
...
*   **Seviye 3 (Level 3):** Derleme platformu sertleştirilmiştir. Derlemeler birbirinden izole çalışır, farklı projelerin derleme süreçlerinin birbirini kirletmesi (cross-contamination) engellenir ve provenans, derlemeyi tetikleyen kullanıcı tarafından taklit edilemez.
...
SBOM, özellikle yeni bir güvenlik açığı (CVE) çıktığında hayati önem taşır. Örneğin, 2021 yılındaki **Log4Shell** krizinde şirketlerin en büyük sorunu, hangi uygulamalarının içinde Log4j kütüphanesinin kullanıldığını bilmemesiydi. SBOM envanterine sahip olan organizasyonlar, tek bir sorguyla hangi yazılımlarının savunmasız olduğunu çok daha hızlı tespit edebilir.

### 5. Pratik Önlemler

*   **Bağımlılıkları sabitleyin:** Kilit dosyaları (lockfile) kullanın ve paketleri sürüm ve hash ile doğrulayın.
*   **İç paketleri koruyun:** Özel paket isimlerini kapsamlı (scoped) adlarla ve iç bir registry üzerinden çözün; böylece bağımlılık karışıklığı saldırısı işe yaramaz.
*   **CI/CD'yi sıkılaştırın:** Derleme hesaplarına en az yetki verin, kısa ömürlü token kullanın ve ana dala doğrudan yazmayı kısıtlayın.
*   **Doğrulayın:** Artefaktları imzalayın (örneğin Sigstore/cosign) ve dağıtımdan önce imzayı doğrulayın.
*   **İzleyin:** SBOM üretin ve bağımlılık taraması (SCA) ile yeni CVE'leri sürekli takip edin.
...
![Diagram](/img/mermaid-software-supply-chain-security-2-bb700d0e.svg)
...
*   **Level 3:** The build platform is hardened. Builds run in isolation from one another, preventing cross-build contamination, and the provenance cannot be forged by the user who triggers the build.
...
The value of an SBOM becomes apparent when a new zero-day vulnerability is announced. During the **Log4Shell** crisis in late 2021, organizations struggled to identify which of their systems contained the vulnerable Log4j library. With an SBOM-indexed software inventory, IT and security teams can query their database and pinpoint vulnerable systems far faster, dramatically reducing response and mitigation times.

### 5. Practical Countermeasures

*   **Pin your dependencies:** Use lockfiles and verify packages by version and hash.
*   **Protect internal packages:** Resolve private package names through scoped names and an internal registry so dependency confusion attacks fail.
*   **Harden CI/CD:** Give build accounts least privilege, use short-lived tokens, and restrict direct writes to the main branch.
*   **Verify:** Sign artifacts (for example with Sigstore/cosign) and verify the signature before deployment.
*   **Monitor:** Generate SBOMs and continuously track new CVEs with software composition analysis (SCA).
