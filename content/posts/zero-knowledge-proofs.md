...
cover:
    ...
    relative: false
related:
  - "[[concepts/zero-knowledge-proofs|Zero-Knowledge Proofs (ZKP)]]"
---
...
Aşağıdaki illüstrasyonda görebileceğiniz gibi, kanıt sunmak isteyen **Kanıtlayıcı** (Prover - tüneldeki karakter) kapının anahtarına sahip olduğunu, dışarıda bekleyen **Doğrulayıcı**ya (Verifier - megafonlu karakter) kanıtlamak istemektedir. Ancak anahtarın kendisini veya gizli kelimeyi doğrulamak amacıyla karşı tarafa vermek istemez.

![İllüstrasyon](/img/xiaohei-zero-knowledge-proofs-1.jpg)
...
1. **Tamlık (Completeness):** Eğer iddia doğruysa ve Kanıtlayıcı dürüstse, dürüst bir Doğrulayıcı her zaman ikna olacaktır.
2. **Sağlamlık (Soundness):** Eğer iddia yanlışsa, hilekar bir Kanıtlayıcı, dürüst bir Doğrulayıcıyı (çok düşük bir şans olasılığı hariç) asla ikna edemez.
...
Modern uygulamalarda etkileşimli süreçler verimsizdir. ZK-SNARKs, kanıtların tek bir adımda gönderilmesini ve çok hızlı doğrulanmasını sağlayan etkileşimsiz yapılardır. Özellikle Zcash gibi gizlilik odaklı blockchain projelerinde ve katman-2 ölçekleme çözümlerinde yaygın olarak kullanılır. Bazı SNARK sistemleri bir "güvenilir kurulum" (trusted setup) aşaması gerektirir; ZK-STARK'lar ise buna ihtiyaç duymaz.
...
As shown in our hand-drawn illustration below, the **Prover** (the character inside the tunnel) wants to prove to the **Verifier** (the character outside with the megaphone) that they know the secret key. However, they refuse to expose the key itself.

![Illustration](/img/xiaohei-zero-knowledge-proofs-1.jpg)
...
2. **Soundness:** If the statement is false, a cheating Prover cannot convince the honest Verifier except with a negligibly small probability.
...
Interactive proofs are slow and inefficient for distributed systems. ZK-SNARKs enable non-interactive proofs that are very short (succinct) and can be verified in milliseconds, making them a good fit for privacy-focused blockchains like Zcash and Ethereum Layer-2 scaling solutions. Some SNARK systems require a "trusted setup" phase; ZK-STARKs do not.
