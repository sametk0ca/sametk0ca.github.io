...
date: 2026-06-20
description: "How race conditions and TOCTOU flaws let attackers abuse concurrency, and how locking, atomic updates and idempotency prevent them. / Race condition ve TOCTOU açıklarının eşzamanlılığı nasıl istismar ettirdiği; kilitleme, atomik güncelleme ve idempotency ile nasıl önlendiği."
draft: false
...
![Diyagram](/img/mermaid-race-condition-1-d4331d33.svg)
...
2. **Yetki Yükseltme (Privilege Escalation):** Tek oturum açma (SSO/OAuth) veya tek kullanımlık şifre (OTP) doğrulama süreçlerinde, doğrulama kodu kontrol edilirken eşzamanlı isteklerle sistemin doğrulanmış durumunu manipüle ederek başka kullanıcıların hesaplarını ele geçirmek mümkündür.
...
* **Atomik İşlemler (Atomic Operations):** Kontrol ve kullanım adımları tek bir bölünmez (atomic) işlem haline getirilmelidir. Örneğin, bakiyeyi kontrol edip sonra düşürmek yerine, veritabanında tek bir `UPDATE cüzdan SET bakiye = bakiye - 100 WHERE id = 1 AND bakiye >= 100` sorgusu çalıştırmak işlemi güvenli kılar (ardından etkilenen satır sayısını kontrol etmeyi unutmayın).
* **Benzersizlik Kısıtları ve Idempotency:** Tek kullanımlık bir kupon için "kullanım" kaydına veritabanı düzeyinde benzersizlik (unique) kısıtı koymak veya isteklere idempotency anahtarı eklemek, aynı işlemin ikinci kez uygulanmasını veritabanına engelletir.
* **İstek Hız Sınırlandırması (Rate Limiting):** Savunma derinliği açısından faydalıdır ancak tek başına yeterli değildir. James Kettle'ın "single-packet attack" tekniği, çok sayıda isteği tek bir TCP paketinde göndererek ağ gecikmesini ortadan kaldırır ve hız sınırlarını atlatabilir. Asıl çözüm, kontrolü ve işlemi sunucu tarafında atomik hale getirmektir.
...
![Diagram](/img/mermaid-race-condition-2-2a009799.svg)
...
* **Atomic Operations:** The verification (Check) and execution (Use) steps must be combined into a single, indivisible (atomic) operation. For example, instead of checking the balance and then deducting it, executing a single update query like `UPDATE wallet SET balance = balance - 100 WHERE id = 1 AND balance >= 100` makes the transaction secure (just remember to check the number of affected rows afterwards).
* **Uniqueness Constraints and Idempotency:** Putting a unique constraint on the "redemption" record of a single-use coupon, or attaching an idempotency key to requests, lets the database itself refuse to apply the same operation twice.
* **Rate Limiting:** Useful as defense in depth but not sufficient on its own. James Kettle's "single-packet attack" sends many requests in a single TCP packet, removing network jitter and bypassing rate limits. The real fix is to make the check and the action atomic on the server side.
