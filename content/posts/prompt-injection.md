...
date: 2026-03-31
description: "How hidden instructions on web pages can hijack AI assistants (indirect prompt injection) and why an AI firewall beats a smarter model. / Web sayfalarındaki gizli talimatların yapay zeka asistanlarını nasıl ele geçirdiği (dolaylı prompt injection) ve neden daha akıllı model yerine yapay zeka güvenlik duvarı gerektiği."
draft: false
tags: ["AI Security", "Prompt Injection", "Large Language Models", "Cybersecurity"]
categories: ["Blog"]
related:
  - "[[concepts/prompt-injection|AI Prompt Injection]]"
---
...
- En önemlisi, web sitelerinden gelen tüm verileri analiz eder. Gizli dolaylı komut enjeksiyonlarını, yapay zeka asistanının mantık yürütme çekirdeğine ulaşmadan önce tespit eder ve engeller.
...
Meta tarafından yapılan bir araştırma, bu saldırıların ne kadar etkili olduğunu ortaya koyuyor. Makaleye göre, dolaylı komut enjeksiyonu saldırıları "vakaların en fazla %86'sında kısmen başarılı" olmuştur. Bu bulgu, asistanların manipülasyona ne kadar açık olduğunu gösterse de, saldırının "kısmen" başarılı olması, saldırganın asıl hedefine çok daha az sıklıkla tam olarak ulaşabildiği anlamına gelir.
...
Research conducted by Meta reveals just how effective these attacks are. According to the paper, indirect prompt injection attacks were "partially successful in up to 86% of cases." While this finding shows how open assistants are to manipulation, the fact that the attack was only "partially" successful means the attacker's real goal was reached in full far less often.
