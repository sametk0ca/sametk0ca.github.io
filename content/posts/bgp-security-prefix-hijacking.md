...
cover:
    ...
    relative: false
related:
  - "[[bgp-security-and-prefix-hijacking|BGP Security and Prefix Hijacking]]"
---
...
Bunu, bir otoyoldaki yön tabelalarını değiştirip trafiği başka bir şehre yönlendiren bir saldırgana benzetebiliriz. Yönlendiriciler önce en spesifik öneki (en uzun önek eşleşmesi - longest prefix match) tercih eder, eşit durumlarda ise BGP'nin yol seçim kurallarına (örneğin daha kısa AS yolu) bakar. Saldırgan bu kuralları kullanarak, örneğin hedefin önekinden daha spesifik bir önek anons ederek trafiği kendi üzerine çekebilir.
...
![Diyagram](/img/mermaid-bgp-security-prefix-hijacking-1-6d676905.svg)
...
*   **ROV (Route Origin Validation):** Yönlendiriciler (routers), aldıkları BGP anonslarını RPKI veri tabanıyla karşılaştırarak doğrular. Eğer anons edilen otonom sistem ROA belgesindekiyle eşleşmiyorsa, anons "Geçersiz" (Invalid) kabul edilir ve reddedilir. Bu sayede sahte orijin anonsları sınırda durdurulmuş olur.
...
This is analogous to someone changing the physical highway signs to redirect traffic meant for a specific city toward an alternate destination. Routers prefer the most specific prefix (longest prefix match) and, among equals, follow BGP's path-selection rules (for example, a shorter AS path). Attackers can exploit these rules, for instance by announcing a more specific prefix than the victim's, to pull traffic toward themselves.
...
![Diagram](/img/mermaid-bgp-security-prefix-hijacking-2-edfb13e2.svg)
...
*   **ROV (Route Origin Validation):** Network operators run local validators that pull ROA data. Routers match incoming BGP announcements against these records. If a route is marked as "Invalid" (meaning the announcing AS does not match the ROA), the announcement is rejected, stopping the hijack at the border.
