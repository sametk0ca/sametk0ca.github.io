...
cover:
    ...
    relative: false
related:
  - "[[concepts/gnss-spoofing-and-takeover-attacks|GNSS Spoofing and Takeover Attacks]]"
---
...
2.  **Zaman Kaydırma (Yanlış Zaman)**: Uydulardaki hassas atomik saatler, kritik altyapıların zaman senkronizasyonu için kullanılır. Saldırgan zaman sinyalini kaydırarak baz istasyonlarının senkronizasyonunu bozabilir, finansal borsalarda işlem zaman damgalarını çarpıtabilir veya elektrik şebekelerindeki faz ölçümlerini bozarak kesintilere yol açabilir.
3.  **Hizmet Dışı Bırakma / Karıştırma (Jamming)**: GPS frekans bandını gürültüyle doldurarak alıcının uydu bağlantısını koparmak ve sistemi GPS'siz bırakmak. Bu, sahte sinyal üretmediği için teknik olarak spoofing'den ayrı bir saldırıdır ama aynı amaçlarla kullanılır.
...
![Diyagram](/img/mermaid-gnss-spoofing-and-takeover-attacks-1-41fc4f34.svg)
...
2.  **Temporal Shift (Incorrect Time)**: GNSS satellites are equipped with highly precise atomic clocks, used worldwide to synchronize critical infrastructure. By shifting the timing signals, an attacker can desynchronize telecommunication towers, distort trade timestamps on financial markets, or disrupt power grid phase measurements, leading to outages.
3.  **Disruption (Jamming)**: Flooding the GPS bands with noise to force the receiver to lose its lock, leaving GPS-based navigation unusable. Because it does not generate fake signals, jamming is technically distinct from spoofing, but it is used for similar goals.
...
![Diagram](/img/mermaid-gnss-spoofing-and-takeover-attacks-2-c6fffeac.svg)
