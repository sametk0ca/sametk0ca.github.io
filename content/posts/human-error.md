...
cover:
    ...
    relative: false
related:
  - "[[concepts/human-error|Human Error and Rule-Bending]]"
---
...
![Diyagram](/img/mermaid-human-error-1-256face6.svg)
...
*   **Sistem 1 (Hızlı Düşünme)**: Otomatik, hızlı, rutin ve çaba gerektirmeyen düşünme biçimidir. Günlük işlerimizin büyük çoğunluğunu bu modda yaparız. Sürekli e-posta yanıtlayan bir çalışan, Sistem 1 modundadır ve gelen her e-postadaki gönderici adresini veya kilit simgesini detaylıca denetlemez.
...
1.  **Uygulanamayacak Politikalar Yazmayın**: Eğer bir şifre politikası 20 karakterli, her ay değişen ve içinde emoji barındıran şifreler istiyorsa, çalışanlar bu şifreyi post-it kağıdına yazıp monitöre yapıştıracaktır. Bu bir kullanıcı hatası değil, tasarım hatasıdır. Nitekim NIST SP 800-63B artık zorunlu periyodik parola değişimini önermiyor.
2.  **Teknik Koruma Ağını Güçlendirin**: Kullanıcıların hata yapabileceğini kabul edin. E-posta ağ geçitlerinde DMARC, DKIM ve SPF doğrulamalarını sıkılaştırarak sahte e-postaların daha kullanıcıya ulaşmadan filtrelenmesini sağlayın.
...
![Diagram](/img/mermaid-human-error-2-645b108e.svg)
...
*   **System 1 (Fast)**: Automatic, rapid, emotional, and efficient. We run the vast majority of our daily tasks on System 1. An employee managing a constant stream of emails relies on System 1, meaning they do not inspect sender domains or lock icons on every incoming message.
...
1.  **Do Not Create Unenforceable Policies**: If a password policy requires a 20-character password changed monthly, users will write it on a sticky note and paste it to their monitors. This is a design failure, not a user failure. NIST SP 800-63B, in fact, no longer recommends mandatory periodic password changes.
