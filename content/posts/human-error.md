...
```mermaid
graph TD
    Threat[Tehdit: Oltalama E-postası] --> Layer1{E-posta Filtresi}
    Layer1 -->|Geçti: Güncellenmemiş Kural Seti| Layer2{Farkındalık Eğitimi}
    Layer2 -->|Geçti: Çalışanın Yorgunluğu/Dikkatsizliği| Layer3{Antivirüs / EDR}
    Layer3 -->|Geçti: Zero-Day Zafiyeti| Incident[Güvenlik İhlali / Sistem Sızması]
    
    style Layer1 fill:#1e1b4b,stroke:#8b5cf6,stroke-width:2px,color:#fff
    style Layer2 fill:#1e1b4b,stroke:#8b5cf6,stroke-width:2px,color:#fff
    style Layer3 fill:#1e1b4b,stroke:#8b5cf6,stroke-width:2px,color:#fff
    style Incident fill:#7c2d12,stroke:#ef4444,stroke-width:2px,color:#fff
```
...
```mermaid
graph TD
    Threat[Threat: Phishing Email] --> Layer1{Email Gateway Filter}
    Layer1 -->|Passed: Outdated Rule Set| Layer2{Security Awareness}
    Layer2 -->|Passed: User Fatigue / Distraction| Layer3{Antivirus / EDR}
    Layer3 -->|Passed: Zero-Day Exploit| Incident[Security Breach / System Compromise]
