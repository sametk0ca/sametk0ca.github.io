...
related:
  ...
  - "[[double-ratchet-algorithm-deep-dive|Double Ratchet]]"
...
Bu tasarım siber güvenlikte **İleri Gizlilik (Forward Secrecy)** sağlar. Bir saldırgan bir şekilde o anki şifreleme anahtarını ele geçirse bile, bu anahtarla geçmiş mesajları açamaz; çünkü eski anahtarlar yok edilmiştir. Double Ratchet'ın Diffie-Hellman adımı sayesinde, saldırgan iletişime aktif olarak müdahale etmediği sürece gelecekteki mesajlar da kısa sürede yeniden güvene alınır (**İhlal Sonrası Güvenlik**). Ayrıntılar için [Double Ratchet yazısına](/posts/double-ratchet-algorithm-deep-dive/) bakabilirsiniz.

### 3. Şifreleme Akışı

Aşağıdaki şemada, bir mesajın şifrelenip iletilmesi ve çözülmesi süreci görselleştirilmiştir:

![Diyagram](/img/mermaid-whatsapp-security-1-e5438284.svg)
...
4.  **Metaveri:** Mesaj içerikleri şifreli olsa da kiminle, ne zaman ve ne sıklıkla yazıştığınız gibi metaveri E2EE'nin kapsamı dışındadır.
...
This structure provides **Forward Secrecy**. If an attacker somehow compromises the current encryption key, they cannot decrypt past messages, because the old keys have been destroyed. Thanks to the Double Ratchet's Diffie-Hellman step, future messages are also quickly made secure again as long as the attacker does not keep actively interfering with the session (**Post-Compromise Security**). See the [Double Ratchet post](/posts/double-ratchet-algorithm-deep-dive/) for details.

### 3. Encryption Workflow

The diagram below visualizes the process of a message being encrypted, transmitted, and decrypted:

![Diagram](/img/mermaid-whatsapp-security-2-c2e50075.svg)
...
4.  **Metadata:** Message contents are encrypted, but metadata such as who you talk to, when, and how often is outside the scope of E2EE.
