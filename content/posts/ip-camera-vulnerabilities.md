...
cover:
    ...
    relative: false
related:
  - "[[concepts/ip-camera-vulnerabilities|IP Camera Vulnerabilities]]"
---
...
*   **Fiziksel Erişim:** Açıkta duran kamera Ethernet kablolarına doğrudan bağlanılarak ağa yetkisiz giriş yapılabilir veya video kayıtlarının saklandığı SD kartlar çalınarak geçmiş görüntülere ulaşılabilir.

![Diyagram](/img/mermaid-ip-camera-vulnerabilities-1-c32b0b0f.svg)
...
6.  **Gereksiz Servisleri Kapatın:** Kullanmadığınız Telnet, SSH veya FTP gibi servisleri devre dışı bırakın; mümkünse yönetim paneline HTTPS ile, video akışına ise şifreli protokollerle (örneğin RTSPS) erişin.
...
Camera security extends beyond the software layer. Attackers can also exploit physical and analog weaknesses:
...
![Diagram](/img/mermaid-ip-camera-vulnerabilities-2-fcabc298.svg)
...
6.  **Turn Off Unneeded Services:** Disable unused Telnet, SSH, or FTP services; where possible, access the admin panel over HTTPS and the video stream over encrypted protocols (for example RTSPS).
