...
cover:
    ...
    relative: false
related:
  - "[[concepts/biometric-authentication-mechanisms|Biometric Authentication Mechanisms]]"
---
...
1.  **Kayıt (Enrollment)**: İlk kullanımda biyometrik veriniz (örneğin parmağınız) taranır. Sistem bu görselden benzersiz özellikleri (parmak izindeki hat birleşimleri ve çatal noktalar gibi) çıkarır ve bunu güvenli bir referans şablonu (template) olarak saklar.
...
![Diyagram](/img/mermaid-biometric-authentication-mechanisms-1-725082c4.svg)
...
*   **Kapasitif ve Isı Sensörleri**: Parmağın elektriksel özelliklerini, sıcaklığını veya nabız ve kan akışını kontrol eder.
...
Canlılık algılamayı tamamlayan bir başka önlem de **FIDO** standartlarıdır (örneğin FIDO UAF). Biyometrik veriniz asla internete veya bir sunucuya gönderilmez; parmak iziniz yalnızca cihazınızdaki özel anahtarın kilidini yerel olarak açar. Sunucuya biyometrik veri yerine, bu anahtarla atılmış kriptografik bir imza gider. Bu sayede sunucular hacklense bile biyometrik verileriniz sızmaz.
...
1.  **Enrollment**: During first-time setup, the sensor scans your biometric feature (e.g., your finger). The system extracts unique micro-features (such as minutiae points, ridge endings, and bifurcations in a fingerprint) and stores them as a secure reference template.
...
![Diagram](/img/mermaid-biometric-authentication-mechanisms-2-9f702f7d.svg)
...
A complementary measure is the **FIDO** family of standards (for example, FIDO UAF, the Universal Authentication Framework). Your raw biometric data never leaves your device: the fingerprint or face scan is processed locally, within a secure enclave, only to unlock a private key. The device then signs a challenge with that key and sends the signature, not the biometric, to the server. Even if the service provider is hacked, your biometric data is not exposed.
